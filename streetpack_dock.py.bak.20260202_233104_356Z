...
TOOLS=['safedel', 'diffwatch', 'dirscope', 'hashscan', 'runshield', 'permcheck', 'secretsniff', 'logtail+', 'netpulse', 'procwatch', 'pathdoctor', 'bulkrename-safe', 'dedupe-lite', 'packreceipt']
...
class App(ttk.Frame):
  def __init__(self, master):
    ...
    self.use_system=tk.BooleanVar(value=False)   # include tools.system.json
...
# SP_TOOLPICKER_METHODS_V1_BEGIN
  ...
  def _receipts_refresh(self):
    try:
      ...
      for f in items:
        self._rcpt_list.insert("end", str(f))
    except Exception:
      pass
  ...
  def _outputs_refresh(self):
    try:
      from pathlib import Path
      p = self._outputs_dir()
      items = sorted(list(p.glob("*.txt")) + list(p.glob("*.log")) + list(p.glob("*.json")), key=lambda x: x.stat().st_mtime, reverse=True)[:250]
      self._out_paths = items
      self._out_list.delete(0, "end")
      for f in items:
        self._out_list.insert("end", str(f))
    except Exception:
      pass
